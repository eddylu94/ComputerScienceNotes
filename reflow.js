...
// Check window size for reflow for two seconds after loading
var reflowCheckCount = 0;
var checkReflow = setInterval(function() {
	reflow();
	reflowCheckCount++;
	if (reflowCheckCount >= 20) {
		clearInterval(checkReflow);
	}
}, 100);

var isMenuFlyoutOpen = false;
var toggleFlyout = function(isOpen) {
	isMenuFlyoutOpen = isOpen == undefined ? !isMenuFlyoutOpen : isOpen;
	var menuFlyout = document.getElementById("menu_flyout");
	var menuFlyoutOverlay = document.getElementById("menu_flyout_overlay");
	if (menuFlyout && menuFlyoutOverlay) {
		menuFlyout.style.display = isMenuFlyoutOpen ? "flex" : "none";
		menuFlyoutOverlay.style.display = isMenuFlyoutOpen ? "flex" : "none";
	}
}
...
var mobilecheck = function() {
	...
	return check || window.innerWidth < 800;
};
